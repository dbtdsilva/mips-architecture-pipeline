...
void hazard::detect_hazard()
{
    ...
    // STALL EXE / BUBBLE ON MEM1
    		...
    		|| (rt_exe.read() != 0 && rt_exe.read() == WriteReg_mem1.read() && MemRead_mem1.read() && RegWrite_mem1.read() && !MemWrite_exe)
    		|| (rs_exe.read() != 0 && rs_exe.read() == WriteReg_mem2.read() && MemRead_mem2.read() && RegWrite_mem2.read())
    		|| (rt_exe.read() != 0 && rt_exe.read() == WriteReg_mem2.read() && MemRead_mem2.read() && RegWrite_mem2.read() && !MemWrite_exe)) 
    {
    	//printf("STALL EXE\n");
    	enable_pc.write(false);
        enable_ifid.write(false);
    ...
    else if (  (rs.read() != 0 && rs.read() == WriteReg_exe.read() && RegWrite_exe.read() && 
    												(Branch.read() || JumpOnRegister.read()))
            || (rt.read() != 0 && rt.read() == WriteReg_exe.read() && RegWrite_exe.read() && Branch.read())

            || (rs.read() != 0 && rs.read() == WriteReg_mem1.read() && MemRead_mem1.read() && RegWrite_mem1.read() && 
            										(Branch.read() || JumpOnRegister.read()))
            || (rt.read() != 0 && rt.read() == WriteReg_mem1.read() && MemRead_mem1.read() && RegWrite_mem1.read() && Branch.read())
            || (rs.read() != 0 && rs.read() == WriteReg_mem2.read() && MemRead_mem2.read() && RegWrite_mem2.read() && 
            										(Branch.read() || JumpOnRegister.read()))
